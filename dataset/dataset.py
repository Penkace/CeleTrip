    ...
    def process(self):
        ...
        for i,(mcnt,mlal,msgl,mndl,medl,mendl) in enumerate(zip(main_cnt_list, main_graph_label_list, main_graph_sub_graph_id, main_graph_node_dict_list, main_graph_event_dict_list, main_graph_entity_dict_list)):
            ...
            for j in msgl:
                ...
                '''
                sub_graph_edge = pd.read_csv(self.sub_graph_save_path+str(j)+'.csv')
...
#         return self.graphs,self.labels,self.event_features,self.entity_features,self.name_features
    ...
    def __getitem__(self,i):
        return self.graphs[i],self.labels[i],np.array(self.sub_graphs[i]),self.name_features[i],self.event_features[i],self.entity_features[i],self.event_freq_features[i],self.entity_graph_list[i],np.array(self.entity_graph_features[i]),torch.from_numpy(np.array(self.location_embedding_static_list[i]))
    
    def __len__(self):
        return len(self.graphs)
    
class Config:
    save_path = '/public/home/pengkai/Itinerary_Miner/CeleTrip/graph_data/path_1/'

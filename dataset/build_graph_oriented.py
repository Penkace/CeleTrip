...
def build_edges(doc_list, word_id_map,id_word_map, vocab, date_word_pair_pim_dict,window_size=15):
...
def generate_graph(data,related_event_sentence,config,max_length = 50,word_embedding_dim=100,is_Train=True):
    ...
    for k in group:
        ...
        #--------------------- 人名index ---------------------#
        node_dict[node_idx] = name_list[0]# 索引——>人名
        name_split_list = name_list[0].split()
        name_split_list = [x for x in name_split_list if len(x)>1]
        node_name = node_idx
        node_idx+=1
        for j in range(node_name):# 连接人名和地名
            row.append(node_name)
            col.append(j)
            row.append(j)
            col.append(node_name)
    
            # 计算人名在sen_list中的频率
            this_loc_sen = sen_list[j]
            name_freq = 0
            for sen in this_loc_sen:
                for na in name_split_list:
                    if na in sen:
                        name_freq+=1
                        break
            name_freq = name_freq / len(this_loc_sen)
            main_weight.append(name_freq)
            main_weight.append(name_freq)
        #----------------------------------------------------#
            ...
            for eve in line:
                ...
                if tup not in event_dict:# 这个事件没有出现过
                    ...
                    if name_flag==1:
                        name_freq = 0
                        one_event_sens_str = related_event_sentence[tup]
                        for sen in one_event_sens_str:
                            for na in sen:
                                name_freq+=1
                                break
                        name_freq = name_freq / len(one_event_sens_str)
...
#             print('Name List: ',name_idx_in_graph)
#             print('Loc List: ',loc_idx_in_graph)
            
#             sub_weight,sub_row,sub_col,sub_number_nodes = build_edges(word_sen_list,vocab,word_id_map,id_word_map,window_size=15)
            sub_row,sub_col,sub_weight,sub_number_nodes= build_edges(word_sen_list, word_id_map,id_word_map, vocab, date_word_pair_pim_dict,window_size=15)
            first_idx_list = []
...
#         print('node dict : ',node_dict)
        ...
        properties_list.append([main_cnt,np.array(label_list),date_graph_list,node_dict,event_dict,entity_dict])
        # --------------------------------------------------------------------- #
    ...
    if is_Train==True:
        ...
        properties_list = pd.DataFrame(properties_list,columns=['graph_id','label','sub_graph_id','node_dict','event_dict','entity_dict'])
